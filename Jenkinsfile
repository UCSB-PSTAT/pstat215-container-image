            ...
            stages{
                ...
                stage('Test') {
                    steps {
                        ...
                        sh 'podman run -it  --rm -v${PWD}/test.stan:/tmp/test.stan:z localhost/pstat217:latest R -e "library(\"rstan\");library(\"tidyverse\");library(\"tidybayes\");sm <- stan_model(\\"/tmp/test.stan\\"); res <- sampling(sm, data=list(N=100, y=rnorm(100)))"'
